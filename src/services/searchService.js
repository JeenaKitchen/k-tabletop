...
class SearchService {
  ...
  async searchRecipes(query) {
    ...
    results.sort((a, b) => {
      const aNameMatch = a.recipe.name.toLowerCase() === normalizedQuery;
      const bNameMatch = b.recipe.name.toLowerCase() === normalizedQuery;
      
      if (aNameMatch && !bNameMatch) return -1;
      if (!aNameMatch && bNameMatch) return 1;
      
      const aNameStartsWith = a.recipe.name.toLowerCase().startsWith(normalizedQuery);
      const bNameStartsWith = b.recipe.name.toLowerCase().startsWith(normalizedQuery);
